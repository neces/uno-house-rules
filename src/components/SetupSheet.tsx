...
/** Cycle through the four Uno colours so neighbours never match (order matches checklist rhythm). */
const SECTION_BULLET_BG = [
  'bg-uno-yellow',
  'bg-uno-red',
  'bg-uno-green',
  'bg-uno-blue',
  ...
  return (
    <article className="rule-sheet space-y-10 sm:space-y-12">
      {content.sections.map((section, idx) => {
        const dot =
          SECTION_BULLET_BG[idx % SECTION_BULLET_BG.length];
        return (
              ...
              {section.items.map((item, i) => {
                return (
                  <li key={i} className="flex items-start gap-3 sm:gap-4">
                    <span
                      aria-hidden
                      className={`shrink-0 w-2.5 h-2.5 rounded-full ${dot} translate-y-[0.55em]`}
                    />
